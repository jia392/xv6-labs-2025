...
| **Lab 2** | **System Calls** | [`syscall-lab`](../../tree/syscall) | 新增系统调用追踪（trace）与内存状态统计（sysinfo） |
| **Lab 3** | **Page Tables** | [`pagetable`](../../tree/pgtbl) | 独立页表改造、`vmprint` 调试打印与用户态访问位检查 |
| **Lab 4** | **Traps** | [`trap`](../../tree/traps) | 栈帧分析、Backtrace 打印与周期性时钟中断回调（Alarm） |
...
| **Lab 8** | **File System** | [`fs`](../../tree/mmap) | 实现内存映射文件系统 `mmap` 及 `munmap` 系统调用 |
